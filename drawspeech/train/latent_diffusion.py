...
def main(configs, config_yaml_path, exp_group_name, exp_name, perform_validation):
    ...
    test_data_subset_folder = os.path.join(
        ...
        "testset_data",
        "_".join(val_dataset.dataset_name) if isinstance(val_dataset.dataset_name, list) else val_dataset.dataset_name
    )
    ...
    num_sanity_val_steps = configs["step"]["num_sanity_val_steps"]
    ...
    monitor = configs["model"]["params"]["monitor"]
    ...
    # print("==> Perform validation every %s steps" % val_check_interval)
